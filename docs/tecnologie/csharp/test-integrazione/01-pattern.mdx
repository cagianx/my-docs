...
Ogni test ha il proprio `IServiceScope`. I servizi si risolvono dallo scope con `Get<T>()`, senza `new` manuale. `Db` e tutti i servizi dello stesso scope condividono il `DbContext`, come avviene in produzione durante una richiesta HTTP.
