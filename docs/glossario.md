...
Termini usati nella documentazione, nel codice e nelle conversazioni di team. Per ogni termine con un sinonimo comune, è indicata la forma preferita nel codice.

---

**ACID**
Proprietà che garantiscono l'affidabilità delle transazioni database: Atomicità, Consistenza, Isolamento, Durabilità. Entity Framework eredita queste garanzie dal database sottostante tramite `SaveChanges()`. Vedi [`regole/entity-framework`](regole/entity-framework.md).
...
**Breaking change**
Modifica che rompe la compatibilità con quanto già in uso: rinomina di colonne o campi API, rimozione di entità, variazione di comportamento atteso. Richiede comunicazione immediata ai team dipendenti e un bump `MAJOR` di versione. Vedi [`regole/versionamento`](regole/versionamento.md).
...
**Clustered index**
Indice che determina l'ordinamento fisico dei dati su disco. In PostgreSQL corrisponde implicitamente alla chiave primaria. Va pianificato in analisi tecnica per supportare il caso d'uso più generico della tabella.
...
Modello usato per trasferire dati tra moduli o attraverso interfacce. Non è mai una copia 1:1 dell'entity del database: espone solo i campi necessari al chiamante, con nomi dall'Ubiquitous Language.
...
**Fluent API**
Metodo di configurazione EF tramite classi `IEntityTypeConfiguration<T>`. Preferito alle Data Annotations perché mantiene le entity class pulite e concentra la configurazione in un unico posto. Vedi [`regole/entity-framework`](regole/entity-framework.md).

---

**Idempotenza**
Proprietà di un'operazione che produce lo stesso risultato indipendentemente da quante volte viene eseguita con gli stessi input. Obbligatoria per operazioni critiche (pagamenti, invio ordini) per gestire retry e timeout. Vedi [`processi/analisi-tecnica/04-contratti`](processi/analisi-tecnica/04-contratti.md).

---

**IUseCase**
Interfaccia marker che identificano formalmente le classi che implementano un caso d'uso. Tutto ciò che implementa `IUseCase` è un caso d'uso; tutto il resto è un servizio che partecipa alla Unit of Work. Vedi [`regole/entity-framework`](regole/entity-framework.md).
...
Requisiti che descrivono *come* il sistema si comporta piuttosto che *cosa* fa: performance, sicurezza, resilienza, tracciabilità, compatibilità. Spesso impliciti nell'analisi funzionale, vanno resi espliciti con metriche verificabili. Vedi [`processi/analisi-tecnica/06-nfr`](processi/analisi-tecnica/06-nfr.md).
...
Principio per cui la struttura del codice comunica immediatamente *cosa fa* il sistema, non *come è costruito*. Le cartelle si chiamano `Ordini/`, `Fatturazione/`, non `Services/`, `Repositories/`. Vedi [`regole/architettura`](regole/architettura.md).
...
**Soft delete**
Tecnica per non eliminare fisicamente un record dal database, ma marcarlo come eliminato tramite un campo (`DeletedAt`, `IsDeleted`). Preserva la storia e la compatibilità con dati storici. Va progettato fin dall'inizio se richiesto.
...
**Unit of Work**
Pattern che raggruppa più operazioni in una singola transazione. In EF, `DbContext` è già una Unit of Work: traccia tutte le modifiche e le persiste atomicamente con `SaveChanges()`. I servizi di dominio partecipano alla Unit of Work senza chiuderla — è il caso d'uso che chiama `SaveChanges()`. Vedi [`regole/entity-framework`](regole/entity-framework.md).
